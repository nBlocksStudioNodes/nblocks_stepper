...
nBlock_STEPPER::nBlock_STEPPER(PinName MOSI, PinName MISO, PinName SCK, PinName pinCS, uint16_t Brightness, uint16_t ScanLimit):
	...
	// Reset flag
	must_update = 0;
    init_STEPPER(Brightness, ScanLimit);		
}
...
void nBlock_STEPPER::endFrame(void){
	if (Position1) {
		Position1 = 0;
		STEPPER_write1(Value1);
    }
	if (Position2) {
		Position2 = 0;
		STEPPER_write2(Value2);
	}	
}

void nBlock_STEPPER::write_STEPPER(uint8_t cmd, uint32_t data) {
    _cs = 0;                         // Set CS Low
...
uint8_t nBlock_STEPPER::read_STEPPER(uint8_t cmd, uint32_t *data) {
...
void nBlock_STEPPER::init_STEPPER(uint8_t cmd, uint32_t data) {
